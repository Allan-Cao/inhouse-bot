...
from inhouse_bot.utils.formatter import sql_role_map
...
class registration_cog(commands.Cog):
    ...
    @discord.slash_command(
        name="link", description="Link your discord with an already registered summoner"
    )
    async def link(self, ctx, ign: str):
        ...
        if user_register != None:
            await ctx.respond(
                f"Your discord account is already connected to the IGN {user_register[0]}. If this is an error, please contact a moderator."
            )
            return
        else:
            ...
            if player_lookup == None:
                await ctx.respond(
                    f"The IGN {ign} is not in the system yet. Please contact a moderator if you think this is an error."
                )
            else:
                update_discord_id(discord_id, ign)
                await ctx.respond(
                    f"Your discord account has been linked to the IGN {ign}"
                )
    ...
    async def secondary(self, ctx, role: str):
        ...
        if player_info == None:
            await ctx.respond(
                f"Your discord account is not linked to any account. Please register yourself into the system before running this command."
            )
            return
        else:
            if player_in_queue != None:
                await ctx.respond(
                    "You are currently in queue, please leave the queue before making role changes"
                )
                return
            ...
            if role == player_info[2]:
                ...
                await ctx.respond(
                    f"Your secondary role has been changed from {player_info[2]} to {role}"
                )
            else:
                change_secondary_role(role, discord_id)
